...
package com.android.settings.luminedroid.edgelight;
...
public class AmbientEdgeLightSettings extends SettingsPreferenceFragment implements
    ...
    private int getColorMode() {
        return Settings.Secure.getIntForUser(getActivity().getContentResolver(),
            LumineSettings.Secure.PULSE_AMBIENT_LIGHT_COLOR_MODE, 1, UserHandle.USER_CURRENT);
    }

    private void setColorMode(int colorMode) {
        Settings.Secure.putIntForUser(getActivity().getContentResolver(),
                LumineSettings.Secure.PULSE_AMBIENT_LIGHT_COLOR_MODE, colorMode, UserHandle.USER_CURRENT);
    }

    private int getSelectedColor() {
        return Settings.Secure.getIntForUser(getActivity().getContentResolver(),
            LumineSettings.Secure.PULSE_AMBIENT_LIGHT_COLOR, 1, UserHandle.USER_CURRENT);
    }

    private void setSelectedColor(int color) {
        Settings.Secure.putIntForUser(getActivity().getContentResolver(),
            LumineSettings.Secure.PULSE_AMBIENT_LIGHT_COLOR, color, UserHandle.USER_CURRENT);
    }
