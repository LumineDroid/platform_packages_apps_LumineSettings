...
public class NetworkTrafficSettings extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mNetTrafficMode = findPreference(LumineSettings.Secure.NETWORK_TRAFFIC_MODE);
        mNetTrafficMode.setOnPreferenceChangeListener(this);
        int mode = Settings.Secure.getInt(resolver,
                LumineSettings.Secure.NETWORK_TRAFFIC_MODE, 0);
        ...
        mNetTrafficPosition = findPreference(LumineSettings.Secure.NETWORK_TRAFFIC_POSITION);
        ...
        int position = Settings.Secure.getInt(resolver,
                LumineSettings.Secure.NETWORK_TRAFFIC_POSITION, POSITION_CENTER);

        if (disallowCenteredTraffic && position == POSITION_CENTER) {
            position = POSITION_END;
            Settings.Secure.putInt(getActivity().getContentResolver(),
                LumineSettings.Secure.NETWORK_TRAFFIC_POSITION, position);
        }
        mNetTrafficPosition.setValue(String.valueOf(position));

        mNetTrafficAutohide = findPreference(LumineSettings.Secure.NETWORK_TRAFFIC_AUTOHIDE);
        mNetTrafficAutohide.setOnPreferenceChangeListener(this);

        mNetTrafficUnits = findPreference(LumineSettings.Secure.NETWORK_TRAFFIC_UNITS);
        mNetTrafficUnits.setOnPreferenceChangeListener(this);
        int units = Settings.Secure.getInt(resolver,
                LumineSettings.Secure.NETWORK_TRAFFIC_UNITS, UNITS_KILOBYTES);
        mNetTrafficUnits.setValue(String.valueOf(units));

        mNetTrafficShowUnits = findPreference(LumineSettings.Secure.NETWORK_TRAFFIC_SHOW_UNITS);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mNetTrafficMode) {
            ...
            Settings.Secure.putInt(getActivity().getContentResolver(),
                    LumineSettings.Secure.NETWORK_TRAFFIC_MODE, mode);
            updateEnabledStates(mode);
            ...
            Settings.Secure.putInt(getActivity().getContentResolver(),
                    LumineSettings.Secure.NETWORK_TRAFFIC_UNITS, units);
            ...
            Settings.Secure.putInt(getActivity().getContentResolver(),
                    LumineSettings.Secure.NETWORK_TRAFFIC_SHOW_UNITS, showUnits);
        }
        return true;
    }
    ...
    private void adjustShowUnitsState(int units, ContentResolver resolver) {
        int showUnits = Settings.Secure.getInt(resolver,
                LumineSettings.Secure.NETWORK_TRAFFIC_SHOW_UNITS, SHOW_UNITS_ON);
