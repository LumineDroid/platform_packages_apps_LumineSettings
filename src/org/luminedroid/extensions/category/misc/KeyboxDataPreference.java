...
public class KeyboxDataPreference extends Preference {
    ...
    deleteButton.setOnClickListener(
        v -> {
          Settings.Secure.putString(
              getContext().getContentResolver(), Settings.Secure.KEYBOX_DATA, null);
  ...
  public void handleFileSelected(Uri uri) {
    ...
    try (InputStream inputStream = getContext().getContentResolver().openInputStream(uri);
      ...
      if (!validateXml(xml)) {
        Toast.makeText(getContext(), "Invalid XML: missing required data", Toast.LENGTH_SHORT)
            .show();
        return;
      }

      Settings.Secure.putString(
          getContext().getContentResolver(), Settings.Secure.KEYBOX_DATA, xml);
