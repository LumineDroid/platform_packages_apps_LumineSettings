...
public class MiscSettings extends SettingsPreferenceFragment
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (mPifJsonManagePreference != null) {
      mPifJsonManagePreference.setOnPreferenceClickListener(
          preference -> {
  ...
  private void downloadAndLoadPifJson(Context context) {
    ...
    DownloadManager.Request request =
        new DownloadManager.Request(uri)
            .setTitle("pif.json")
            .setDescription(
                context.getResources().getString(R.string.spoofing_pif_json_download_message))
  ...
  private void loadPifJson(Uri uri) {
    try (InputStream inputStream = getActivity().getContentResolver().openInputStream(uri)) {
      if (inputStream != null) {
        ...
        Toast.makeText(getContext(), R.string.spoofing_pif_json_select_success, Toast.LENGTH_LONG)
            .show();
      }
    } catch (Exception e) {
      Log.e(TAG, "Error reading JSON or setting properties", e);
      Toast.makeText(getContext(), R.string.spoofing_pif_json_select_failure, Toast.LENGTH_LONG)
          .show();
    }
  }
